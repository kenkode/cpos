...
class Jcart {
	...
	* @return mixed
	*/
	private function add_item($id, $name, $price, $qty = 1, $url, $size) {
	...
	*/
	public function display_cart() {
		...
		// Add an item
		if ($_POST[$add]) {
			$itemAdded = $this->add_item($id, $name, $price, $qty, $url, $size);
		...
		echo tab(1) . "<form method='post' id='form'>\n";
		echo tab(2) . "<fieldset>\n";
		echo tab(3) . "<input type='hidden' name='jcartToken' value='{$_SESSION['jcartToken']}' />\n";
		echo tab(3) . "<table class='panel-heading' border='0' style='width:430px !important; border-top-right-radius: 10px; border-top-left-radius: 10px !important; border-spacing:0 5px !important; border-collapse:separate; margin-top:-13px'>\n";
		echo tab(4) . "<thead style='background: #2389bb; color: #fff; line-height: 40px;'>\n";
		echo tab(5) . "<tr>\n";
		echo tab(6) . "<th colspan='6'>\n";
		echo tab(7) . "<strong style='margin-left:10px !important' id='jcart-title'>{$config['text']['cartTitle']}</strong> ($this->itemCount $itemsText)\n";
		echo tab(6) . "</th>\n";
		echo tab(5) . "</tr>". "\n";
		echo tab(4) . "</thead>\n";
		...
		echo tab(4) . "<input type='$inputType' id='clearCart' $src name='jcartEmpty' value='{$config['text']['emptyButton']}' class='jcart-button'/>\n";

			if ($config['button']['checkout']) {
				$inputType = "image";
				$src = " src='{$config['button']['checkout']}' alt='{$config['text']['checkout']}' title='' ";
			}
			echo tab(7) . "<input type='$inputType' class='btn btn-info sc-cart-checkout' $src id='jcart-checkout' name='jcartCheckout' class='jcart-button' value='{$config['text']['checkout']}' />\n";
		}
		...
		echo tab(4) . "<input type='$inputType' disabled id='clearCart' $src name='jcartEmpty' value='{$config['text']['emptyButton']}' class='jcart-button'/>\n";
			...
			echo tab(7) . "<input type='$inputType' disabled class='btn btn-info sc-cart-checkout' $src id='jcart-checkout' name='jcartCheckout' class='jcart-button' value='{$config['text']['checkout']}' />\n";
		}
		...
		echo tab(4) . "<tbody style='background:#fff'>\n";
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				echo tab(6) . "</td>\n";
				echo tab(6) . "<td class='jcart-item-remove'>\n";
		...
		if ($isCheckout === true) {
			...
			echo tab(3) . "<input type='$inputType' $src id='jcart-paypal-checkout'  class='btn btn-info sc-cart-checkout' name='jcartPaypalCheckout' value='{$config['text']['checkoutPaypal']}' $disablePaypalCheckout />\n";
		}
